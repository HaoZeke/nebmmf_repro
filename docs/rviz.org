...
(let* ((root (vc-root-dir))
       (bin-dir (expand-file-name ".pixi/envs/rviz/bin" root))
...
df_dumbbell <- data_raw %>%
  ...
  pivot_longer(
    cols = c(Calls_CINEB_Plot, Calls_OCINEB_Plot, Time_CINEB_Plot, Time_OCINEB_Plot),
...
p_dumbbell <- ggplot() +
 ...
 scale_y_discrete(labels = scales::label_parse(), expand = expansion(mult = c(0.05, 0.05))) + 
  scale_color_manual(
    values = c("CINEB" = ruhi_colors[["coral"]], "OCINEB" = ruhi_colors[["teal"]])
  ) +
  scale_shape_manual(
...
scale_x_log10(
  ...
  labs(
    ## title = "Performance Gap: OCINEB vs. CINEB",
    ## subtitle = "Labels indicate savings (negative values = OCINEB faster).",
    x = "Log Scale Value",
    y = NULL,
    color = "Method",
    shape = "Status"
...
#+RESULTS[f604b3f525745cdf9f6eefcb355f442eb622f8dc]:
...
df_plot_ready <- df_long %>%
  filter(Method %in% c("CINEB", "OCINEB"), success == TRUE)
...
#+RESULTS[d4f28369d4e4a6e458fb1ed2b963ea399947301a]:
...
# This validates that OCINEB isn't just 'fast' because it's finding wrong/lazy saddles.
...
## ggsave("replot.png", p_combined, width=14, height=12,dpi=300)
...
#+RESULTS[90c37126aab2726ecfd069981d9d2d4e42aaf715]:
