...
#+begin_src R :results discard
data_raw <- process_transition_state_data("../benchmark_results.csv")
##  Need a long version too
dff <- data_raw %>%
  pivot_longer(
    cols = matches("_(CINEB|MMF)$"),
    names_to = c(".value", "Method"),
    names_pattern = "(.*)_(CINEB|MMF)"
  ) %>%
  mutate(
    ...
    Method = factor(Method, levels = c("CINEB", "MMF"))
  )
#+end_src
...
df_long <- data_raw %>%
  ...
  mutate(
    ...
    Method = ifelse(Method == "MMF", "RONEB", Method),    
...
#+RESULTS[eee09c218f0effc884f99947f341b2bdf30f87a6]:
...
#+RESULTS[a29dfdb8d056ad13d53f309e749db7eaf1c16802]:
#+begin_example
 Family: negbinomial 
...
sds(sRMSD_Init_FinalMethodCINEB_1)     0.67      0.61     0.02     2.22 1.00
sds(sRMSD_Init_FinalMethodRONEB_1)     0.63      0.58     0.02     2.13 1.00
                                   Bulk_ESS Tail_ESS
sds(sRMSD_Init_FinalMethodCINEB_1)    10827    11028
sds(sRMSD_Init_FinalMethodRONEB_1)    12279    11499

Multilevel Hyperparameters:
~System (Number of levels: 24) 
              Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
sd(Intercept)     0.43      0.09     0.28     0.62 1.00     4800     8431

Regression Coefficients:
                               Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS
Intercept                          6.38      0.11     6.16     6.59 1.00     6754
shape_Intercept                    2.60      0.41     1.80     3.42 1.00     6989
MethodRONEB                       -0.62      0.08    -0.79    -0.46 1.00    23393
shape_MethodRONEB                  0.12      0.46    -0.81     1.03 1.00    13564
sRMSD_Init_Final:MethodCINEB_1     0.54      0.11     0.32     0.76 1.00     6188
sRMSD_Init_Final:MethodRONEB_1     0.55      0.10     0.34     0.76 1.00     6241
                               Tail_ESS
Intercept                         10286
shape_Intercept                   12216
MethodRONEB                       16755
shape_MethodRONEB                 15483
sRMSD_Init_Final:MethodCINEB_1     8380
sRMSD_Init_Final:MethodRONEB_1     7688
...
#+begin_src R :results value :colnames yes :post round-tbl[:colnames yes](*this*)
param_draws_eff <- brms_roneb %>%
  tidy_draws() %>%
  select(b_Intercept, b_MethodRONEB, `sd_System__Intercept`)
...
summary_mmf_eff <- param_draws_eff %>%
  mutate(mult = exp(b_MethodRONEB), perc = (mult - 1) * 100) %>%
...
formatted_mmf <- bind_rows(
  summary_mmf_eff %>% mutate(Effect_Type = "Multiplicative Factor (RONEB vs CINEB)", 
                             `Median Effect` = sprintf("%.2f", med_m),
                             `95% CrI` = paste0("[", sprintf("%.2f", lo_m), ", ", sprintf("%.2f", hi_m), "]")),
  summary_mmf_eff %>% mutate(Effect_Type = "Percentage Change (RONEB vs CINEB)", 
...
#+RESULTS[379772df734dfba2e93530c593c42b4706e5d729]:
| Effect_Type                            | Median Effect | 95% CrI          |
|----------------------------------------+---------------+------------------|
| Expected PES Calls (Baseline: CINEB)   |         587.4 | [473.9, 725.7]   |
| Multiplicative Factor (RONEB vs CINEB) |           0.5 | [0.45, 0.63]     |
| Percentage Change (RONEB vs CINEB)     |        -46.3% | [-54.7%, -36.9%] |
...
#+RESULTS[18ae98255ec8bd65435b57366e3f29f1aeb449c2]:
...
#+RESULTS[bc786aa8d7db4a558bdecd91d04e40c66650843b]:
...
pp_check(brms_roneb, type = "intervals_grouped", group = "Method") +
...
#+RESULTS[9de2b02aebc279bbc1766c58f91ca840dde42043]:
...
shape_draws <- brms_roneb %>%
  as_draws_df() %>%
  mutate(
    `Shape (CINEB)` = b_shape_Intercept,
    `Shape (RONEB)` = b_shape_Intercept + b_shape_MethodRONEB
  ) %>%
  pivot_longer(cols = c(`Shape (CINEB)`, `Shape (RONEB)`), 
               names_to = "Algorithm", 
...
#+RESULTS[4b3ef2c155e62a81cd89e3d84b11a8e4bad58941]:
...
# Compute LOO
loo_result <- loo(brms_roneb, save_psis=TRUE, reloo = TRUE)
loo_result$estimates
#+end_src

#+RESULTS[e93157bc4cdd77db771eb2025ec32f5a0b7cf329]:
|          | Estimate |   SE |
|----------+----------+------|
| elpd_loo |   -312.8 |  9.3 |
| p_loo    |     24.6 |  4.0 |
...
#+RESULTS[3205d61cf86c69a7dea694b3c307d729a62785a3]:
#+begin_example
...
#+RESULTS[228b667f17aa388a7b511856f6d6e93966d57785]:
...
cond_effects <- conditional_effects(brms_roneb, effects = "RMSD_Init_Final:Method", points = TRUE)
...
p_linear_robustness <- ggplot(plot_data, aes(x = RMSD_Init_Final, y = estimate__, color = Method, fill = Method)) +
  ...
  labs(
    ...
    x = expression("Distance (" * RMSD[Init - TS] * " in Å)"),
...
#+RESULTS[8359ca64b387c9018ca32d6b093250da2713d78a]:
